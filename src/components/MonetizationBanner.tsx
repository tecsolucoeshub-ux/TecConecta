...
export const MonetizationBanner: React.FC<MonetizationBannerProps> = ({
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2 sm:py-3">
      <div
        className={`relative rounded-2xl overflow-hidden border p-3 sm:p-4 transition-all duration-300 ${
          isLight
            ? 'bg-gradient-to-r from-orange-50/90 via-white to-cyan-50/80 border-orange-200/90 shadow-sm'
            : 'border-[#FF6B00]/30 bg-gradient-to-r from-[#0C1530] via-[#111F45] to-[#1A1A3A] shadow-[0_0_25px_rgba(255,107,0,0.1)]'
        }`}
      >
        {/* Glow Accent */}
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-12 gap-3 sm:gap-4 items-center">
          {/* Left Column: Sponsored Company (if any) or Main Ad Call */}
          <div className="lg:col-span-8 flex items-start sm:items-center gap-3">
            {/* Visual Icon or Business Photo Showcase */}
            {currentBanner?.imageUrl && !imageErrorMap[currentBanner.id] ? (
              <div className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-xl overflow-hidden border-2 border-[#00E5FF]/50 shadow-[0_0_15px_rgba(0,229,255,0.2)] shrink-0 bg-black/40 group">
                <img
                ...
                <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent py-0.5 text-center">
                  <span className="text-[8px] font-bold text-[#00E5FF] tracking-wider uppercase">Destaque</span>
                </div>
              </div>
            ) : currentBanner ? (
              <div className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-xl overflow-hidden border border-[#00E5FF]/40 shrink-0 shadow-sm">
                <img
                  src={generateFallbackBrandImage(currentBanner.companyName, currentBanner.category)}
                  alt={currentBanner.companyName}
              ...
              <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-[#FF6B00]/25 to-[#00E5FF]/20 border border-[#FF6B00]/40 flex items-center justify-center text-[#FF6B00] shadow-[0_0_15px_rgba(255,107,0,0.2)] shrink-0">
            ...
            {currentBanner ? (
              <div className="space-y-0.5 text-left flex-1 min-w-0">
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className="text-[9.5px] font-extrabold uppercase tracking-wider px-1.5 py-0.5 rounded bg-[#FF6B00]/20 text-[#FF6B00] border border-[#FF6B00]/40">
                    {currentBanner.badgeText || 'Patrocinador'}
                  </span>
                  ...
                  {activeBanners.length > 1 && (
                    <span className={`text-[9.5px] font-mono ${isLight ? 'text-slate-500' : 'text-gray-400'}`}>
                ...
                <h3
                  className={`font-['Outfit'] text-sm sm:text-base font-bold leading-tight truncate ${
              ...
              <div className="space-y-0.5 text-left flex-1 min-w-0">
                ...
                <h3
                  className={`font-['Outfit'] text-sm sm:text-lg font-extrabold leading-tight ${
                    isLight ? 'text-slate-900' : 'text-white'
                  }`}
                >
                ...
                <p className={`text-xs ${isLight ? 'text-slate-600' : 'text-gray-300'}`}>
                  {adminSettings.bannerSubtext || 'Entre em contato com o departamento comercial'}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-4 flex flex-row items-center justify-end gap-2 flex-wrap sm:flex-nowrap">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 sm:flex-none px-3.5 py-2 rounded-xl bg-gradient-to-r from-[#FF6B00] to-orange-500 hover:from-orange-500 hover:to-[#FF6B00] text-white font-bold text-xs font-['Outfit'] flex items-center justify-center gap-1 shadow-[0_0_15px_rgba(255,107,0,0.3)] transition transform hover:-translate-y-0.5 text-center whitespace-nowrap"
